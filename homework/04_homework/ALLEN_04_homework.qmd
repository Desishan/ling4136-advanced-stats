...
# YOUR CODE HERE
glmdl1 <- glmer(chosen_voicing ~ correct_voicing +
                  # `correct_voicing` varies within a given listener, item pair,
                  # and voice, so all of these should have random slopes
...
`Error in glmer(chosen_voicing ~ correct_voicing + (1 + correct_voicing |  :    unused argument (REML = FALSE)`

So I took it out. Not sure what that does.
...
# YOUR CODE HERE
# Double-checking which level is above on the y axis. Looks like it's `voiceless`.
levels(incomplete$chosen_voicing)

# Viewing the model coefficients
summary(glmdl1)

# Transforming estimated values into the probability space
plogis(c(-0.02, (-0.02 + 0.52)))

# Creating a null model
glmdl_reduced <- glmer(chosen_voicing ~ 1 +
                  # `correct_voicing` varies within a given listener, item pair, and voice, so all of these should have random slopes
                (1 + correct_voicing | listener) +
                (1 + correct_voicing | item_pair) +
                (1 + correct_voicing | voice),
               data = incomplete,
...
# YOUR CODE HERE
predictions <- as.data.frame(predict_response(model = glmdl1))

# It looks like predict_response has converted the log odds to probability,
# which is convenient.
ggplot(data = predictions,
       aes(x = correct_voicing.x,
           y = correct_voicing.predicted)) +
  geom_errorbar(aes(ymin = correct_voicing.conf.low,
                  ymax = correct_voicing.conf.high),
              colour = 'red') +
  geom_point(size = 5) +
  labs(title = '\nPredicted proportion phonemes percieved as voiceless vs.\n
       ...
       Error bars represent a 95% confidence interval.\n',
         x = '\nintended voicing',
         y = 'proportion pervceived as voiceless (predicted)\n') +
  theme_minimal()
...
# Computing averages for each listener in a new dataframe
averages_listener <- incomplete |>
  group_by(listener, correct_voicing) |>
  summarise(prop_perceived_voiceless = sum(chosen_voicing == 'voiceless')/n())

# Plotting
consistency_listener <- ggplot(data = averages_listener,
                      aes(x = correct_voicing,
                          y = prop_perceived_voiceless,
                          group = listener,
                          colour = listener)) +
  geom_point() +
  geom_line(alpha = 0.5) +
  labs(title = 'Variation in the relationship between correct and perceived voicing\nby listener\n',
       x = '\nproportion perceived as voiceless',
       y = 'intended voicing\n') +
  theme_minimal()

# Same again, for item pair
averages_item <- incomplete |>
  group_by(item_pair, correct_voicing) |>
  summarise(prop_perceived_voiceless = sum(chosen_voicing == 'voiceless')/n())

consistency_item <- ggplot(data = averages_item,
                      aes(x = correct_voicing,
                          y = prop_perceived_voiceless,
                          group = item_pair,
                          colour = item_pair)) +
...
# Same again, for speaker voice
averages_speaker <- incomplete |>
  group_by(voice, correct_voicing) |>
  summarise(prop_perceived_voiceless = sum(chosen_voicing == 'voiceless')/n())

consistency_speaker <- ggplot(data = averages_speaker,
                      aes(x = correct_voicing,
                          y = prop_perceived_voiceless,
                          group = voice,
                          colour = voice)) +
...
# Display variation plots
consistency_listener
consistency_item
consistency_speaker
```

This reflects pretty intuitively the pattern given in the model's random effects: listeners are the most consistent, though there is some intercept and slope variation; there is even one listener whose slop is nearly zero.

By item pair, there is a lot more variation, and while slopes generally trend positive, some are even negative.

By speaker, both slopes and intercepts vary massively, though it looks like only three of speaker voices actually show a negative slope. In fact, those three speakers' slopes appear so close to each other that I wonder if they possess a particular common feature, say of dialect or voice quality. I also wonder if there is a ceiling effect present, given the positive slopes towards the higher voiceless proportions seem typically flatter than the lower-intercept ones.
